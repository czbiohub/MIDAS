    ...
    for s in iggdb.species_info:
        try:
            ...
            if gdim == "pangenomes":
                ...
                # this is too long for Prokka
                #    >4547837|first_part_from_repgnome_file_before_space
                # ? how to do thi/^>/ s/ .*//s?
                #
                # where
                #
                ...
                s_header_xform = f"sed 's=^>=>{s_species_alt_id}|=' {s_repgenome_path} > {s_repgenome_path_clean} && cat {s_repgenome_path_clean} >> {outdir}/temp_{gdim}.fa && echo SUCCEEDED || echo FAILED"
